...
Процедура ВыгрузитьТоварыНаСервере()
	 ...
	 Если Объект.тТекущаяКонфигурация = "КомплекснаяАвтоматизация"  Тогда
		 
		 тзМаг = ПолучитьСписоокАктивныхМагазиновКомплексная(); 
	...
	КонецЦикла; 
Иначе 
	Сообщить("Выгрузку весов можно производить только из центрального узла!",СтатусСообщения.Важное);
	 КонецЕсли;
...
КонецЕсли;
ФайлСДанными.Записать("D:\TeleportIP\" + тЗначение + "\весы1", КодировкаТекста.ANSI);
ФайлСДанными.Записать("D:\TeleportIP\" + тЗначение + "\весы2", КодировкаТекста.ANSI);
ФайлСДанными.Записать("D:\TeleportIP\" + тЗначение + "\весы3", КодировкаТекста.ANSI);
...
//
&НаСервере
Функция ПолучитьСписоокАктивныхМагазиновКомплексная()
	...
	Запрос.Текст = 
		...
		|	Др.Значение КАК Значение,
		|	Магазины.Ссылка КАК Ссылка,
		|	Магазины.УчетныйВидЦены КАК ВидЦен
		|ИЗ
